...
He simply mixes two colors for the sky, one is for normal sky color and the other one is the more greyish horizon color. He gets the mix factor (x) from the y coordinate of the pixel and gives it the value \(e^{-10.0*x}\).
